...
class Clear(commands.Cog):
    ...
    async def clear(self, ctx, option=None, num_messages: int = None):
        if option == "purge":
            ...
            await ctx.send(f"{ctx.author.mention}, veuillez spécifier une option valide (ex: `purge`).")
        else:
    ...
    async def clear_error(self, ctx, error):
        if isinstance(error, commands.MissingPermissions):
            ...
            await ctx.send(f"{ctx.author.mention}, argument invalide. Utilisez `j!clear purge` pour purger le canal.")
        else:
